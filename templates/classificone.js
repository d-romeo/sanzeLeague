    ...
    <v-container>
     ...
     <v-row justify="center" v-if="liveMatchData">
        ...
        <v-row justify="center">
            <v-col cols="12">
                <v-card class="pa-6 zoom-card" style="background-color: rgba(0, 0, 0, 0.7);" @click="goToRank"  hover ripple dark>
                <v-container class="pa-0">
                    ...
                    <!-- DATI DAL DB -->
                    <v-row v-for="(team, index) in classificaDB.slice(0, 4)" :key="team.id_rank" class="align-center" no-gutters style="margin-bottom: 8px;">
                        <v-col cols="1" class="text-center text-subtitle-1 font-weight-bold">{{ team.id_rank }}</v-col>
                        <v-col cols="2">
                            <v-img :src="team.logo_path" contain max-width="36" max-height="36"></v-img>
                        </v-col>
                        <v-col cols="5" class="text-subtitle-1 font-weight-medium">{{ team.name }}</v-col>
                        <v-col cols="2" class="text-center text-subtitle-1">{{ team.punti }}</v-col>
                        <v-col cols="2" class="text-center text-subtitle-1">{{ team.df_reti }}</v-col>
                    </v-row>
    ...
    methods: {
        caricaClassificaDB() {
            ...
            .then(data => {
                ...
                console.error('Errore caricamento classifica:', data);
                }
            })
            .catch(error => {
        ...
        caricaPartiteDB() {
            ...
            .then(data => {
                ...
                this.nextMatches = this.nextMatches.filter(match => match.dateObj > now );
            })
            .catch(err => {
